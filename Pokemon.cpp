...
double Pokemon::MakeAttack(Move attack, string opp_type)
{
    ...
    if (hitRoll > attack.acc) return 0.0; //miss
        ...
        double typeEff = TypeMap[opp_type]; //look up type effectiveness, not sure how I want to pass the attack's type to the defending pokemon's takeDamage
...
{
    hp -= rawDamage;//will probably pass attack type to here, then
}
